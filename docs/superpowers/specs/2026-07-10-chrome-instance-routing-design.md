...
The selected behavior is: target the frontmost visible non-debug Chrome instance, reuse a tab with the same navigation identity inside that process, and use the official Chrome launcher to open a missing URL in the ordinary Chrome session. The cmux remote-debugging instance must never receive the target.

## Considered Approaches

1. **PID-targeted lookup with an official-launcher fallback (selected).** Discover ordinary Chrome PIDs, choose the frontmost visible instance, connect through `SBApplication(processIdentifier:)` for navigation-identity lookup, and use the official Chrome executable only when the URL is missing. This preserves URL-only configuration and avoids the cmux remote-debugging instance without a browser extension.
...
- Rust passes the selected PID, configured URL, and normalized match prefix as `osascript` arguments.
- The script creates `SBApplication(processIdentifier:)` for that PID.
- It reads each window's tab URLs in one batch per window.
- Rust removes query and fragment components from the match prefix. For paths containing `/merges/<numeric-id>`, it also removes later view segments, so the MR root, Files, Commits, and anchored variants share one navigation identity.
- The script accepts an exact match or a prefix followed by `/`, `#`, or `?`, which preserves URL boundaries and avoids matching an adjacent identifier such as `73420` when the target is `7342`.
...
- If no matching navigation identity exists, the script returns `NOT_FOUND`. Rust then launches `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome --new-tab <original-url>`. In the supported ordinary-plus-cmux-debug setup, Chrome routes this request to the ordinary default session; the isolated remote-debugging instance remains untouched.
...
- Static-script tests verify that PID, URL, and match prefix are arguments, tab URLs are read in batches, AppKit activates the selected PID, matching respects URL boundaries, and user values never enter source.
- URL normalization tests cover query/fragment removal and the shared identity of MR root, Files, and Commits views.
...
- Live acceptance runs with both the ordinary Chrome and cmux debug Chrome present: an existing matching URL variant must focus in the ordinary instance; a missing URL must open there; no test tab or permission process may remain afterward.

## Out of Scope

- Choosing a specific Chrome profile within one ordinary process.
- Guaranteeing the fallback's exact ordinary PID when multiple independent non-debug Chrome roots use different user-data directories; existing-tab lookup remains PID-specific.
